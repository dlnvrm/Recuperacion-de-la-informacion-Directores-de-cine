# evaluacion.py
#
# Calcula Precisión y Recall del sistema booleano sobre las 20
# necesidades de información definidas en datos/relevancias.json.
#
# ─────────────────────────────────────────────────────────────────
# MÉTRICAS
#
...
#
# Ambas métricas van de 0 (peor) a 1 (mejor).
# Se reportan como promedios sobre las 20 necesidades.
# ─────────────────────────────────────────────────────────────────
...
def cargar_relevancias() -> dict:
    """Lee el fichero JSON con las consultas booleanas y los documentos relevantes."""
    with open(RUTA_RELEVANCIAS, 'r', encoding='utf-8') as f:
        return json.load(f)


def calcular_precision_recall(
        recuperados: list, relevantes: list) -> Tuple[float, float]:
    """
    Dados los documentos recuperados y los relevantes, devuelve
    (precisión, recall).
    """
    rec = set(recuperados)
    rel = set(relevantes)
...
def evaluar() -> Dict:
    ...
    Para cada necesidad:
      1. Lee la consulta booleana del fichero de relevancias.
      2. Ejecuta la búsqueda con buscar().
      3. Calcula precisión y recall comparando con los relevantes marcados.

    Devuelve un dict con los resultados por necesidad y los promedios.
    """
    datos      = cargar_relevancias()
    resultados = {}
    precisiones, recalls = [], []

    print(f"\n{'Necesidad':<46} {'P':>6}  {'R':>6}  "
          f"{'VP':>4}  {'Rec':>4}  {'Rel':>4}")
    print('─' * 76)
